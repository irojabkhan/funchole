...
export function LogTraceConsole({ invocationId, currentStatus, inspection, onRefresh, refreshing }: LogTraceConsoleProps) {
  ...
  return (
    <div className="rounded-3xl border border-border bg-[#08080a] p-4">
      <div className="flex flex-col gap-3 border-b border-border pb-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-muted">Invocation trace</p>
          <div className="mt-2 flex flex-wrap items-center gap-2">
            <StatusBadge status={currentStatus} />
            <code className="break-all rounded-full border border-border bg-surface px-2.5 py-1 font-mono text-[11px] text-muted-strong">{invocationId}</code>
          </div>
        </div>
      ...
      <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          {(["all", "stdout", "stderr", "errors"] as LogFilter[]).map((item) => (
            <button
              ...
              className={`rounded-full border px-3 py-1 text-xs font-semibold capitalize transition-colors ${
                filter === item ? "border-accent-border bg-accent-soft text-accent" : "border-border text-muted hover:text-foreground"
              }`}
            >
              {item}
            </button>
        ...
        <input
          ...
          className="h-9 rounded-full border border-border bg-surface px-3 font-mono text-xs text-foreground outline-none transition-colors placeholder:text-muted/60 focus:border-accent"
        />
      </div>

      {!inspection ? (
        <div className="mt-4 rounded-2xl border border-dashed border-border bg-surface/40 p-5">
          <p className="text-sm font-semibold text-foreground">Waiting for dispatcher and runtime output</p>
          <p className="mt-1 text-xs leading-5 text-muted">The invocation was accepted. Logs and result will appear here after the runtime reports back.</p>
        </div>
      ) : (
        <div className="mt-4 space-y-3">
          {inspection.error && (
            <pre className="whitespace-pre-wrap rounded-2xl border border-danger/30 bg-danger/10 p-3 font-mono text-xs leading-5 text-danger">{inspection.error}</pre>
          )}
          {inspection.result && (
            <pre className="max-h-72 overflow-auto whitespace-pre-wrap rounded-2xl border border-success/25 bg-success/10 p-3 font-mono text-xs leading-5 text-muted-strong">{inspection.result}</pre>
          )}
          {filteredSteps.length === 0 || filteredSteps.every((step) => step.logs.length === 0 && !step.error) ? (
            <div className="rounded-2xl border border-dashed border-border bg-surface/40 p-5">
              <p className="text-sm font-semibold text-foreground">No logs match this view</p>
              <p className="mt-1 text-xs text-muted">Try clearing the search or switching back to all logs.</p>
            </div>
          ) : (
  ...
  return (
    <div className="rounded-2xl border border-border bg-surface/70 p-3">
      <div className="flex items-center gap-2">
        <span className={`h-2.5 w-2.5 rounded-full ${active ? "bg-accent" : "bg-muted/40"} ${pulsing ? "fh-pulse" : ""}`} />
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-border bg-surface/70">
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border px-3 py-2">
        <div className="flex items-center gap-2">
          <span className="rounded-full border border-border bg-surface-2 px-2 py-0.5 font-mono text-[11px] text-muted">#{step.position}</span>
          <p className="text-sm font-semibold text-foreground">{step.componentType}</p>
        </div>
        ...
        <pre className="max-h-72 overflow-auto p-3 font-mono text-[11px] leading-5">
          {step.logs.map((log, index) => (
            <div key={`${log.createdAt}-${index}`} className={log.stream === "stderr" ? "text-danger" : "text-muted-strong"}>
              <span className="select-none text-muted">{log.stream.padEnd(6)} </span>
              {log.message}
            </div>
          ))}
        </pre>
