...
public class S3SourceStore implements SourceStore {
    ...
    public void save(UUID functionVersionId, List<SourceFile> files) {
        ...
        ConcurrentIo.forEach(files, file -> {
        ...
        List<S3Object> staleObjects = listResponse.contents().stream()
                .filter(existing -> !newKeys.contains(existing.key()))
                .toList();
        ConcurrentIo.forEach(staleObjects, existing -> s3Client.deleteObject(DeleteObjectRequest.builder()
                .bucket(bucket)
                .key(existing.key())
                .build()));
    }
    ...
    public List<SourceFile> load(UUID functionVersionId, List<String> relativePaths) {
        return ConcurrentIo.map(relativePaths, relativePath -> loadOne(functionVersionId, relativePath));
    }

    private SourceFile loadOne(UUID functionVersionId, String relativePath) {
